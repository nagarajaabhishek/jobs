...
- Every major claim affecting score MUST have an `evidence[]` item with an exact `jd_quote`.
- `score_breakdown` MUST sum to `apply_conviction_score`.
- If JD text is too short/noisy or you cannot find direct quotes, set `jd_quality: low`, reduce `confidence`, and keep verdict conservative.
